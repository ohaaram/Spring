...
public class User2Service {
    ...
    public ResponseEntity<?> selectUser2(String uid){
        ...
        try {
            //orElseThrow()메서드로 존재하는 Entity를 가져오거나 존재하지 않으면 기본 예외 noSuchElementException
    ...
    public ResponseEntity<List<User2DTO>> selectUser2s(){
        List<User2DTO> user2DTOs = repository.findAll()
                .stream()
    ...
    public ResponseEntity deleteUser2(String uid){
        ...
        if(optUser2.isPresent()){
            ...
            //사용자가 존재하지 않으면 NOT_FOUND 응답데이터와 user no found 메세지
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body("user not found");
        }
    }
}
